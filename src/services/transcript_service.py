...
def get_ytt():
    from src.config import PROXY_USERNAME, PROXY_PASSWORD
...
def _fallback_to_audio(url):
    """No captions available — download audio and transcribe with Whisper."""
    text, error = transcribe_video_audio(url)
    if text:
        return text, "No captions found — transcribed from audio instead (Whisper)."
    return None, f"No captions found, and audio transcription also failed: {error}"
